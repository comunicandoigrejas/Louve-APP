...
# 1. CONFIGURAÇÃO DA PÁGINA E CSS (Esconde botões superiores e rodapé)
...
# 2. BRANDING NA SIDEBAR (Visível antes do login)
...
try:
    ...
    st.error("Erro nos Secrets. Verifique as chaves OpenAI e Google JSON.")
    st.stop()
...
def carregar_louvores():
    try:
        ...
        return df
    except: return pd.DataFrame(columns=["Musica", "Artista", "Tom", "Andamento", "Categoria"])
    ...
    if chave_mestre == "shekina123":
        ...
        with t2:
            ...
            with st.form("form_cad", clear_on_submit=True):
                nova_m = st.text_input("Nome da Música:")
                novo_a = st.text_input("Artista/Ministério:")
                novo_t = st.text_input("Tom Sugerido:")
                if st.form_submit_button("Salvar Música"):
                    if nova_m and novo_a:
                        df_atual = carregar_louvores().drop(columns=['Musica_Busca'], errors='ignore')
                        novo_df = pd.DataFrame([[nova_m, novo_a, novo_t, "Médio", "Geral"]], columns=["Musica", "Artista", "Tom", "Andamento", "Categoria"])
        ...
        with t3:
            ...
            m_para_excluir = st.selectbox("Selecione a música para remover:", [""] + sorted(df_l['Musica'].tolist()))
            if m_para_excluir and st.button("Confirmar Exclusão Permanente"):
                df_restante = df_l[df_l['Musica'] != m_para_excluir].drop(columns=['Musica_Busca'], errors='ignore')
                conn.update(worksheet="Louvores", data=df_restante)
                st.warning(f"Música '{m_para_excluir}' removida.")
                st.rerun()
        ...
        with t5:
            ...
            if st.button("✨ Gerar Mensagem"):
                with st.spinner("Criando devocional de 200 palavras..."):
                    p = f"Escreva um devocional de 200 palavras sobre {tema_c if tema_c else 'Adoração'}. Use emojis: 🙏, ✨, 📖. Sem triângulos."
                    res = client.chat.completions.create(model="gpt-3.5
