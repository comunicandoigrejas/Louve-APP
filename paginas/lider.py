...
# Carregamento de dados locais para esta página
try:
    ...
    if nome_c and final_list:
        ...
        with col_save:
            if st.button("💾 PUBLICAR E SALVAR PROGRAMAÇÃO"):
                ...
                st.success("✅ Programação gravada e publicada com sucesso!")
        with col_wa:
...
with t3:
    ...
    excluir = st.selectbox("Selecione para remover permanentemente:", [""] + sorted(df_l['Musica'].tolist()))
...
with t5:
    ...
    if 'dev_final' in st.session_state:
        ...
        st.markdown(f'<a href="{link_dev}" target="_blank"><button style="width:100%; background-color:#25D366; color:white; border:none; padding:10px; border-radius:8px; font-weight:bold; cursor:pointer;">📢 ENVIAR DEVOCIONAL PRO WHATSAPP</button></a>', unsafe_allow_html=True)
